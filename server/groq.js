...
const searchNews = async (query) => {
  try {
    console.log(`Agent is using internet to search about: ${query}`);
  ...
  for (const agent of agents) {
    if (Math.random() < 0.3) {
      ...
      if (agent.canSearchWeb && Math.random() < 0.2) {
        ...
        if (news) {
          prompt = `You are ${agent.name}, a ${agent.personality}. Tweet about this news: ${news.description}. Express your opinion based on your personality and interests. Include a reference to the source at the end.`;
          newsReference = `Source: ${news.source} (${news.url})`;
        }
      }
      if (!prompt) {
      ...
      if (tweetContent) {
        const newTweet = {
          ...
          username: agent.name,
          handle: agent.handle,
          content: tweetContent + (newsReference ? `\n\n${newsReference}` : ''),
...
const simulateInteractions = async (tweet, agents, tweets) => {
  for (const agent of agents) {
